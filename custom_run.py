...
from datasets.combined_dataset.train_magnification import magnification_factors
...
def main():
    ...
    # args = dict(model='yolov8s.pt', data='datasets\combined_dataset\data.yaml', imgsz=256, epochs=50, device=0)

    # 2. resume run*
    # args = dict(model='runs/detect/train2/weights/last.pt', data='datasets\combined_dataset\data.yaml', imgsz=256, epochs=50, device=0, resume=True)
    # trainer = CustomTrainer(overrides=args)
    # trainer.train()

    # model test
    model = YOLO("runs/detect/train2/weights/best.pt")
    model.val(data='datasets\combined_dataset\data.yaml', plots=True, split='test')
