...
import random
import yaml
from dataset_path import YAML_PATH, DEFAULT_AFTER_PATH, DATASET_DIR, TRAIN_PATH, VALID_PATH, TEST_PATH
...
def process_class_data(class_name):
    image_path = os.path.join(DEFAULT_AFTER_PATH, class_name, 'images')
    image_files = os.listdir(image_path)

    # Seed the random number generator for reproducibility
    random.seed(42)  # You can choose any number as your seed
    ...
    def copy_files(files, data_type):
        for file in files:
            # Copy image
            image_from_class_path = os.path.join(DEFAULT_AFTER_PATH, f'{class_name}/images/{file}')
            ...
            label_from_class_path = os.path.join(DEFAULT_AFTER_PATH, f'{class_name}/labels/{label_file}')
